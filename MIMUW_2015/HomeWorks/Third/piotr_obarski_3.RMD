...
Jeśli zależność pomiędzy zmiennymi jest liniowa model powinnien być całkowicie dopasowany. Zatem zróbmy właśnie w ten sposób.
```{r}
library(ggplot2)
library(lmtest)
...
```
Widać na wykresach diagnostycznych, że model jest dobry. Co ciekawe wykresy nie są całkowicie zdegenerowane i nie są "perfekcyjne", a późniejsze spojrzenie na residua pokazuje, że są one bliskie 0, a nie tak jak powinny być równe 0.
</br>
...
Spotkało mnie tutaj niemiłe zaskoczenie, gdyż jednak model, który był testowany jako pierwszy nie ma reszt rozłożonych normalnie, mimo że jest idealna zależność liniowa. Dodam zatem do y1 z dataset1 trochę zaburzenia normalnego i powtórzę testy.


```{r}
y3<-y1+rnorm(1000)*10
dataset3<-data.frame(x1,y3)
model<-lm(y3~x1, data=dataset3)
shapiro.test(rstandard(model))
shapiro.test(rstandard(model2))
```
...
```{r}
plot(x2~y2, data=dataset2)
par(mfrow=c(1,2))
hmctest(y3~x1, order.by=~x1, data = dataset3, plot=TRUE)
...
Teraz testuję liniowość testem Rainbow. Za zbiór danych który ma przejść test przyjmuję dataset3. Lekko zmodyfikuję zbiór dataset2 i on będzie tym, który testu nie przejdzie.
```{r}
dataset2$y2[200:500]<- -dataset2$y2[200:500]
plot(y2~x2, data=dataset2)
raintest(y3~x1, order.by=~x1, data = dataset3)
