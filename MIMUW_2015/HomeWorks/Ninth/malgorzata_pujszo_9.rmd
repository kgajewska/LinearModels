...
qplot(sigma2.1^2, geom="histogram") + ggtitle("Histogram of sigma1^2")
summary(sigma2.1^2)
```

As we can see this distribution is skewed with the maximum value around 16 but it is mostly concentrated near 0.
...
qplot(sigma2.2^2, geom="histogram") + ggtitle("Histogram of sigma1^2")
summary(sigma2.2^2)
```

It is also a skewed distribution but here it is more spread.