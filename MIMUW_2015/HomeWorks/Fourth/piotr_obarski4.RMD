...
for (i in 1:1000) {
  ...
  x[which(levels2=="cc")]<-rnorm(400)+0.7
  x[which(levels=="d")]<-rnorm(400)+0.7
...
```
Jako że nie ma interakcji wydaje się, że test na interakcje powinnien być zaakceptowany w ok. 95% przypadków, jednak tak się nie stało. </br>
## Punkt 2
...
```
Tutaj sytuacja analogiczna jak wyżej, jednak zarówno efekty interakcji jak i główne są obecne.
