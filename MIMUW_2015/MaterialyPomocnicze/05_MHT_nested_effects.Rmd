...
Let assume that male have blood pressure higher than female and the difference is delta1 = 1. Then let's assume that nested effect of diabetes is delta2= (0, 0, 0.5) for male and delta3=(0, 0, 1) for female.

Our goal is to find sample size, for which the test for nested effect (only effect of diabetes) has power 0.58.

* Create a simulation study in which for a given n - sample size you will calculate power for hypothesis that delta2=0 and delta3=0. 
* Choose the smallest n in a way that you will have power not smaller than 0.5. I.e. what is the minimal number of patients that allow to detect effect of the size (0,0,0.5,0,0,1) with probability larger than 80%.
